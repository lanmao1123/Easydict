...
class Annotation {
    ...
    var randomSeed: UInt32 = .random(in: 1 ... UInt32.max)
    ...
    var supportsRotation: Bool {
        switch tool {
        case .ellipse, .filledRectangle, .number, .rectangle, .stamp, .text:
            return true
        default:
            return false
        }
    ...
    static func approxBezierLength(
        from p0: NSPoint,
        cp1: NSPoint,
        cp2: NSPoint,
        to p3: NSPoint,
    ...
    func hitTest(point: NSPoint, threshold: CGFloat = 8) -> Bool {
        ...
        case .blur, .highlight, .pixelate, .stamp:
    ...
    private static func smoothPathLength(_ pts: [NSPoint]) -> CGFloat {
        ...
        for s in 1 ... steps {
            ...
            let u = 1 - localT
            let px = u * u * u * p1.x + 3 * u * u * localT * cp1.x + 3 * u * localT * localT * cp2
                .x + localT * localT * localT * p2.x
            let py = u * u * u * p1.y + 3 * u * u * localT * cp1.y + 3 * u * localT * localT * cp2
                .y + localT * localT * localT * p2.y
    ...
    private func drawFreeform(alpha: CGFloat, width: CGFloat) {
        ...
        if lineStyle == .dotted {
            ...
            for i in 1 ..< points.count {
                cumLengths.append(cumLengths[i - 1] + hypot(
    ...
    private func drawArrow() {
        ...
        let ep1 = NSPoint(
            x: lastPt.x - arrowLen * cos(endAngle - arrowAngle),
        ...
        if arrowStyle == .double {
            ...
            sp1 = NSPoint(
                x: firstPt.x - arrowLen * cos(startAngle - arrowAngle),
        ...
        if lineStyle != .solid {
            let length = hasMultiAnchor ? Self.smoothPathLength(pts) :
                (controlPoint != nil ? Annotation.approxBezierLength(
                    from: lineStart,
                    cp1: controlPoint!,
                    cp2: controlPoint!,
                    to: endBase
        ...
        if let oc = outlineColor {
            ...
            case .sketchy, .thick: break // handled by early returns above
            }
            if arrowStyle == .tail {
                let cr = NSRect(
                    x: firstPt.x - tailRadius - outlineW,
                    y: firstPt.y - tailRadius - outlineW,
                    width: (tailRadius + outlineW) * 2,
                    height: (tailRadius + outlineW) * 2
                )
        ...
        if arrowStyle == .tail {
            let circleRect = NSRect(
                x: firstPt.x - tailRadius,
                y: firstPt.y - tailRadius,
                width: tailRadius * 2,
                height: tailRadius * 2
            )
    ...
    private func drawThickArrow() {
        ...
        // Head base point
        let headBase = NSPoint(
            x: lastPt.x - headLen * cos(endAngle),
    ...
    private func drawRectangle(forceFilled: Bool = false) {
        ...
        let cornerRadius: CGFloat = rectCornerRadius > 0 ? rectCornerRadius :
            (isRounded ? min(rect.width, rect.height) * 0.2 : 0)
    ...
    private func drawNumber() {
        ...
        if dist > radius + outlineWidth {
            ...
            let fillBaseL = NSPoint(
                x: center.x + baseHalfWidth * cos(perpAngle),
                y: center.y + baseHalfWidth * sin(perpAngle)
            )
            let fillBaseR = NSPoint(
                x: center.x - baseHalfWidth * cos(perpAngle),
                y: center.y - baseHalfWidth * sin(perpAngle)
            )
    ...
    private func bakeErase() -> NSImage? {
        ...
        let regionImage = NSImage(size: sampleRect.size, flipped: false) { _ in
            src.draw(
                in: NSRect(
                    x: -(sampleRect.minX - srcBounds.minX),
    ...
    private func drawLoupe(in context: NSGraphicsContext) {
        ...
        if let src = loupeSourceRect, src.width > 4, src.height > 4 {
            ...
            let srcSquare = NSRect(
                x: srcCenter.x - srcSize / 2,
                y: srcCenter.y - srcSize / 2,
                width: srcSize,
                height: srcSize
            )
        ...
        if let baked = bakedBlurNSImage {
            baked.draw(
                in: squareRect,
                from: NSRect(origin: .zero, size: baked.size),
                operation: .sourceOver,
                fraction: 1.0
            )
