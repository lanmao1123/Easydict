...
class EffectsPickerView: NSView {
    ...
    var onConfigChanged: ((ImageEffectsConfig) -> ())?
    ...
    private func setupUI() {
        ...
        contrastSlider = addSliderRow(
            label: L("Contrast"),
            y: &y,
            min: 0.5,
            max: 2.0,
            value: Double(config.contrast),
            action: #selector(contrastChanged(_:))
        )
        saturationSlider = addSliderRow(
            label: L("Saturation"),
            y: &y,
            min: 0.0,
            max: 2.0,
            value: Double(config.saturation),
            action: #selector(saturationChanged(_:))
        )
        sharpnessSlider = addSliderRow(
            label: L("Sharpness"),
            y: &y,
            min: 0.0,
            max: 2.0,
            value: Double(config.sharpness),
            action: #selector(sharpnessChanged(_:))
        )
    ...
    @objc
    private func brightnessChanged(_ sender: NSSlider) {
    ...
    @objc
    private func contrastChanged(_ sender: NSSlider) {
    ...
    @objc
    private func saturationChanged(_ sender: NSSlider) {
    ...
    @objc
    private func sharpnessChanged(_ sender: NSSlider) {
