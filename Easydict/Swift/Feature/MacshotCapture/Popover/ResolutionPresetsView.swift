...
final class ResolutionPresetsView: NSView {
    ...
    @objc
    private func keepRatioChanged(_ sender: NSSwitch) { onToggleKeepRatio?(sender.state == .on) }
    @objc
    private func unitChanged(_ sender: NSSegmentedControl) { onPickUnit?(sender.selectedSegment) }
}

// MARK: - ResolutionPresetRow
