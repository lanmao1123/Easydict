...
class ColorPickerView: NSView {
    ...
    var onColorChanged: ((NSColor) -> ())?
    /// Called whenever opacity changes.
    var onOpacityChanged: ((CGFloat) -> ())?
    ...
    var onCustomSlotSelected: ((Int) -> ())?
    /// Called when custom colors change (for persistence).
    var onCustomColorsChanged: (([NSColor?]) -> ())?
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        cursorY -= padding
        customSlotRects = []
        let totalCustomW = CGFloat(customColors.count) * customSlotSize + CGFloat(customColors.count - 1) *
            customSlotSpacing
        ...
        let oRect = NSRect(
            x: padding,
            y: cursorY - opacityBarHeight,
            width: pickerWidth - padding * 2,
            height: opacityBarHeight
        )
    ...
    private func drawOpacitySlider(in rect: NSRect) {
        ...
        for ci in 0 ... cols {
            for ri in 0 ... 1 {
                ...
                NSRect(
                    x: rect.minX + CGFloat(ci) * checkSize,
                    y: rect.minY + CGFloat(ri) * checkSize,
                    width: checkSize,
                    height: checkSize
                ).fill()
            }
        ...
        NSGradient(starting: selectedColor.withAlphaComponent(0), ending: selectedColor.withAlphaComponent(1))?.draw(
            in: path,
            angle: 0
        )
    ...
    private func drawHexDisplay(in rect: NSRect) {
        ...
        (hex as NSString).draw(
            at: NSPoint(x: hashX + hashSize.width, y: rect.midY - hashSize.height / 2),
