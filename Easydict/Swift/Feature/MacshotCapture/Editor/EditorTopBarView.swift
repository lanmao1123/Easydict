...
class EditorTopBarView: NSView {
    ...
    @objc
    private func zoomButtonClicked() {
        let menu = NSMenu()
    ...
    @objc
    private func zoomPresetAction(_ sender: NSMenuItem) {
    ...
    @objc
    private func flipHClicked() { overlayView?.flipImageHorizontally() }
    @objc
    private func flipVClicked() { overlayView?.flipImageVertically() }
    @objc
    private func addCaptureClicked() { overlayView?.overlayDelegate?.overlayViewDidRequestAddCapture() }
}
