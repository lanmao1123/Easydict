...
class ToolOptionsRowView: NSView {
    ...
    func rebuild(for tool: AnnotationTool) {
        ...
        if tool == .pencil {
            ...
            let seg = NSSegmentedControl(
                labels: [L("None"), L("Smooth"), L("Refined")],
    ...
    private func addStrokeSlider(at x: CGFloat, tool: AnnotationTool, ov: OverlayView) -> CGFloat {
        ...
        let slider = NSSlider(
            ...
            minValue: tool == .loupe ? 40 : 1,
            maxValue: tool == .loupe ? 320 : 30,
            target: self,
    ...
    private func addLineStyleSegment(at x: CGFloat, ov: OverlayView) -> CGFloat {
        ...
        let hasOutline = editingAnnotation?
            .outlineColor != nil || (isShapeTool && UserDefaults.standard.bool(forKey: "annotationOutlineEnabled"))
    ...
    @objc
    private func piiSegmentClicked(_ sender: NSSegmentedControl) {
    ...
    private func addToggle(at x: CGFloat, title: String, isOn: Bool, action: @escaping (Bool) -> ()) -> CGFloat {
        var curX = x
    ...
    private func addNumberOptions(at x: CGFloat, ov: OverlayView) -> CGFloat {
        ...
        let seg = NSSegmentedControl(
            ...
            trackingMode: .selectOne,
            target: self,
            action: #selector(numberFormatChanged(_:))
        )
    ...
    private func addTextOptions(at x: CGFloat, ov: OverlayView) -> CGFloat {
        ...
        fillLabelBtn.frame = NSRect(
            x: curX,
            y: (rowHeight - 22) / 2,
            width: max(30, fillLabelBtn.frame.width),
            height: 22
        )
        addSubview(fillLabelBtn)
        curX += fillLabelBtn.frame.width + 2

        let fillSwatch = NSButton(frame: NSRect(
            x: curX,
            y: (rowHeight - fillSwatchSize) / 2,
            width: fillSwatchSize,
            height: fillSwatchSize
        ))
        fillSwatch.title = ""
        ...
        outlineLabelBtn.frame = NSRect(
            x: curX,
            y: (rowHeight - 22) / 2,
            width: max(50, outlineLabelBtn.frame.width),
            height: 22
        )
        addSubview(outlineLabelBtn)
        curX += outlineLabelBtn.frame.width + 2

        let outlineSwatch = NSButton(frame: NSRect(
            x: curX,
            y: (rowHeight - fillSwatchSize) / 2,
            width: fillSwatchSize,
            height: fillSwatchSize
        ...
        strokeLabelBtn.frame = NSRect(
            x: curX,
            y: (rowHeight - 22) / 2,
            width: max(46, strokeLabelBtn.frame.width),
            height: 22
        )
        addSubview(strokeLabelBtn)
        curX += strokeLabelBtn.frame.width + 2

        let strokeSwatch = NSButton(frame: NSRect(
            x: curX,
            y: (rowHeight - fillSwatchSize) / 2,
            width: fillSwatchSize,
            height: fillSwatchSize
    ...
    private func addMeasureToggle(at x: CGFloat, ov: OverlayView) -> CGFloat {
        ...
        let seg = NSSegmentedControl(
            ...
            trackingMode: .selectOne,
            target: self,
            action: #selector(measureUnitChanged(_:))
        )
        ...
        curX = addToggle(
            at: curX,
            title: L("Limit to selection"),
            isOn: ov.currentMeasureClampToSelection
        ) { [weak ov] isOn in
    ...
    private func addRedactOptions(at x: CGFloat, ov: OverlayView) -> CGFloat {
        ...
        let drawSeg = NSSegmentedControl(
            labels: [L("All"), L("Text Only")],
            trackingMode: .selectOne,
            target: self,
        ...
        curX = addRedactButton(
            at: curX,
            title: L("All Text"),
            action: #selector(redactAllTextClicked),
            font: btnFont,
            height: btnH,
            y: btnY
        )
        ...
        curX = addRedactButton(
            at: curX,
            title: L("PII"),
            action: #selector(redactPIIClicked),
            font: btnFont,
            height: btnH,
            y: btnY,
        ...
        curX = addRedactButton(
            at: curX,
            title: L("Faces"),
            action: #selector(redactFacesClicked),
            font: btnFont,
            height: btnH,
            y: btnY
        )
        ...
        curX = addRedactButton(
            at: curX,
            title: L("People"),
            action: #selector(redactPeopleClicked),
            font: btnFont,
            height: btnH,
            y: btnY
        )
    ...
    private func addBeautifyOptions(at x: CGFloat, ov: OverlayView) -> CGFloat {
        ...
        if !isSnap {
            let modeSeg = NSSegmentedControl(
                ...
                trackingMode: .selectOne,
                target: self,
                action: #selector(beautifyModeChanged(_:))
            )
        ...
        if !isSnap {
            curX = addBeautifySlider(
                ...
                min: 0,
                max: 30,
                action: #selector(beautifyCornerChanged(_:))
            )
        }
        ...
        curX = addBeautifySlider(
            ...
            min: 0,
            max: 100,
            action: #selector(beautifyShadowChanged(_:))
        )
        ...
        if ov.beautifyStyleIndex == -1 {
            curX = addBeautifySlider(
                ...
                min: 0,
                max: 50,
                action: #selector(beautifyBlurChanged(_:))
            )
        }
        ...
        let swatchBtn = NSButton(frame: NSRect(
            x: curX,
    ...
    private func addBeautifySlider(
        at x: CGFloat,
        label: String,
        value: CGFloat,
        min: CGFloat,
    ...
    @objc
    private func beautifyModeChanged(_ sender: NSSegmentedControl) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func beautifyPaddingChanged(_ sender: NSSlider) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func beautifyCornerChanged(_ sender: NSSlider) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func beautifyShadowChanged(_ sender: NSSlider) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func beautifyBlurChanged(_ sender: NSSlider) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func beautifyGradientClicked(_ sender: NSButton) {
    ...
    @objc
    private func beautifyToggleChanged(_ sender: NSButton) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func strokeSliderChanged(_ sender: NSSlider) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func stampSizeChanged(_ sender: NSSlider) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func loupeMagnificationChanged(_ sender: NSSlider) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func highlightDimChanged(_ sender: NSSlider) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func highlightBorderChanged(_ sender: NSSegmentedControl) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func lineStyleChanged(_ sender: NSSegmentedControl) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func arrowStyleChanged(_ sender: NSSegmentedControl) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func shapeFillChanged(_ sender: NSSegmentedControl) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func cornerRadiusChanged(_ sender: NSSlider) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func censorModeChanged(_ sender: NSSegmentedControl) {
    ...
    @objc
    private func numberFormatChanged(_ sender: NSSegmentedControl) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func numberStartChanged(_ sender: NSStepper) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func boldToggled() {
        overlayView?.textEditor.toggleBold(); overlayView
            .map { $0.applyTextFormattingToSelectedAnnotations(); $0.needsDisplay = true; rebuild(for: $0.currentTool) }
    }

    @objc
    private func italicToggled() {
        overlayView?.textEditor.toggleItalic(); overlayView
            .map { $0.applyTextFormattingToSelectedAnnotations(); $0.needsDisplay = true; rebuild(for: $0.currentTool) }
    }

    @objc
    private func underlineToggled() {
        overlayView?.textEditor.toggleUnderline(); overlayView
            .map { $0.applyTextFormattingToSelectedAnnotations(); $0.needsDisplay = true; rebuild(for: $0.currentTool) }
    }

    @objc
    private func strikethroughToggled() {
        overlayView?.textEditor.toggleStrikethrough(); overlayView
            .map { $0.applyTextFormattingToSelectedAnnotations(); $0.needsDisplay = true; rebuild(for: $0.currentTool) }
    }

    @objc
    private func measureUnitChanged(_ sender: NSSegmentedControl) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func quickEmojiClicked(_ sender: NSButton) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func moreEmojisClicked(_ sender: NSButton) {
        if PopoverHelper.toggleClosedIfOpen() { return }
        guard let ov = overlayView else { return }
    ...
    @objc
    private func drawModeChanged(_ sender: NSSegmentedControl) {
        UserDefaults.standard.set(sender.selectedSegment == 1, forKey: "censorTextOnly")
    }

    @objc
    private func pencilSmoothModeChanged(_ sender: NSSegmentedControl) {
    ...
    @objc
    private func redactAllTextClicked() {
        overlayView?.performRedactAllText()
    }

    @objc
    private func redactPIIClicked() {
        overlayView?.performAutoRedact()
    }

    @objc
    private func redactFacesClicked() {
        overlayView?.performRedactFaces()
    }

    @objc
    private func redactPeopleClicked() {
        overlayView?.performRedactPeople()
    }

    @objc
    private func redactTypesClicked(_ sender: NSView) {
        if PopoverHelper.toggleClosedIfOpen() { return }
        guard let ov = overlayView else { return }
    ...
    @objc
    private func fontFamilyClicked(_ sender: NSButton) {
    ...
    @objc
    private func alignmentChanged(_ sender: NSButton) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func fontSizeDecreased() {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func fontSizeIncreased() {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func textBgToggled(_ sender: NSButton) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func textOutlineToggled(_ sender: NSButton) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func textBgColorClicked(_ sender: NSButton) {
        if PopoverHelper.toggleClosedIfOpen() { return }
        guard let ov = overlayView else { return }
    ...
    @objc
    private func textOutlineColorClicked(_ sender: NSButton) {
        if PopoverHelper.toggleClosedIfOpen() { return }
        guard let ov = overlayView else { return }
    ...
    @objc
    private func textGlyphStrokeToggled(_ sender: NSButton) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func textGlyphStrokeColorClicked(_ sender: NSButton) {
        if PopoverHelper.toggleClosedIfOpen() { return }
        guard let ov = overlayView else { return }
    ...
    @objc
    private func annotationOutlineToggled(_ sender: NSButton) {
        guard let ov = overlayView else { return }
    ...
    @objc
    private func annotationOutlineColorClicked(_ sender: NSButton) {
        if PopoverHelper.toggleClosedIfOpen() { return }
        guard let ov = overlayView else { return }
    ...
    @objc
    private func loupeOutlineToggled(_ sender: NSButton) {
        guard let ov = overlayView else { return }
        let isOn = sender.state == .on
    ...
    @objc
    private func loupeOutlineColorClicked(_ sender: NSButton) {
        if PopoverHelper.toggleClosedIfOpen() { return }
        guard let ov = overlayView else { return }
    ...
    @objc
    private func textCancelClicked() {
        overlayView?.cancelTextEditing()
    }

    @objc
    private func textConfirmClicked() {
...
private class ToggleHandler: NSObject {
    ...
    @objc
    func toggled(_ sender: NSButton) { action(sender.state == .on) }
}
