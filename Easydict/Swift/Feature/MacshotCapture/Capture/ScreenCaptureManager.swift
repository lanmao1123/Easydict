...
class ScreenCaptureManager {
    ...
    /// as focus changes.
    static func makeImmediateCaptureContext(timing: (@Sendable (String) -> ())? = nil) -> ImmediateCaptureContext {
    ...
    static func captureAllScreensImmediatelySCK(
        ...
        timing?(
            "SCK immediate: shareable content end displays=\(content.displays.count) windows=\(content.windows.count)"
        )
    ...
    static func captureAllScreens(
        ...
        timing: (@Sendable (String) -> ())? = nil,
        completion: @escaping ([ScreenCapture]) -> ()
    ) {
        Task {
            do {
                ...
                if !excludingWindowNumbers.isEmpty {
                    ...
                    timing?(
                        "SCShareableContent fresh end displays=\(content.displays.count) windows=\(content.windows.count)"
                    )
                } else {
                    timing?("SCShareableContent cached begin")
                    content = try await shareableContent()
                    timing?(
                        "SCShareableContent cached end displays=\(content.displays.count) windows=\(content.windows.count)"
                    )
                }
                    ...
                    for (index, pair) in pairs.enumerated() {
                        ...
                        group.addTask {
                            if #available(macOS 14.0, *) {
                                ...
                                timing?(
                                    "SCScreenshotManager capture end display=\(index) pixels=\(image.width)x\(image.height)"
                                )
                                return ScreenCapture(screen: screen, image: image)
                            } else {
                                ...
                                timing?(
                                    "fallback CGWindowListCreateImage end display=\(index) pixels=\(image.width)x\(image.height)"
                                )
                                return ScreenCapture(screen: screen, image: image)
                            }
                        }
    ...
    static func captureWindow(windowID: CGWindowID, screen: NSScreen) async -> CGImage? {
        ...
        if #available(macOS 14.0, *) {
            ...
            guard let scWindow = content.windows.first(where: { CGWindowID($0.windowID) == windowID })
            else { return captureViaWindowList() }
        ...
        let captures = await withTaskGroup(
            ...
            for (index, screen) in screens.enumerated() {
                group.addTask {
                    ...
                    timing?(
                        "SCK rect capture begin screen=\(index) rect=\(Int(rect.origin.x)),\(Int(rect.origin.y)) \(Int(rect.width))x\(Int(rect.height))"
                    )
    ...
    private static func captureScreenshotOutput(
        ...
        -> (output: SCScreenshotOutput?, error: Error?) {
