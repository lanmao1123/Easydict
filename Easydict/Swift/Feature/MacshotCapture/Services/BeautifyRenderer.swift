...
class BeautifyRenderer {
    ...
    static let styles: [BeautifyStyle] = {
        ...
        s.append(contentsOf: [
            ...
            BeautifyStyle(
                stops: [(c(0.7, 0.1, 0.2), 0), (c(0.9, 0.2, 0.3), 0.5), (c(0.55, 0.05, 0.15), 1)],
                angle: 150
            ),
            BeautifyStyle(
                stops: [(c(0.1, 0.15, 0.5), 0), (c(0.2, 0.3, 0.75), 0.5), (c(0.05, 0.1, 0.4), 1)],
                angle: 120
            ),
            BeautifyStyle(
                stops: [(c(0.85, 0.75, 0.55), 0), (c(0.95, 0.88, 0.7), 0.5), (c(0.75, 0.65, 0.45), 1)],
                angle: 135
            ),
            BeautifyStyle(
                stops: [(c(0.95, 0.95, 0.95), 0), (c(1.0, 1.0, 1.0), 0.5), (c(0.92, 0.92, 0.92), 1)],
                angle: 180
            ),
            BeautifyStyle(
                stops: [(c(0.05, 0.05, 0.05), 0), (c(0.12, 0.12, 0.12), 0.5), (c(0.0, 0.0, 0.0), 1)],
                angle: 180
            ),
        ])
    ...
    static func drawRoundedImageWithShadow(
        _ image: NSImage,
    ...
    static func drawGradientBackground(
        in rect: NSRect,
        config: BeautifyConfig,
        context: CGContext,
        prerenderedMesh: CGImage? = nil
    ) {
        ...
        if config.isCustomBackground {
            ...
            if let bgImage = config.customBackgroundImage {
                ...
                if config.backgroundBlur > 0, let cgImg = bgImage.cgImage(
                    forProposedRect: nil,
                    context: nil,
                    hints: nil
                ) {
        ...
        let scale = max(
            abs(dx) > 0.001 ? halfW / abs(dx) : .greatestFiniteMagnitude,
    ...
    private static func meshStyle(
        points: [SIMD2<Float>],
        colors: [NSColor],
        fallbackStops: [(NSColor, CGFloat)],
        fallbackAngle: CGFloat = 135
    )
        -> BeautifyStyle {
        BeautifyStyle(
            stops: fallbackStops,
    ...
    private static func renderWindow(image: NSImage, config: BeautifyConfig) -> NSImage {
        ...
        let result = NSImage(size: NSSize(width: totalWidth, height: totalHeight), flipped: false) { _ in
            ...
            let titleBarRect = NSRect(
                x: windowX,
                y: windowY + windowHeight - titleBarHeight,
                width: windowWidth,
