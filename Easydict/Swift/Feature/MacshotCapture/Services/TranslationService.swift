...
enum TranslationService {
    ...
    static func checkAppleLanguageAvailability(completion: @escaping ([String: Bool]) -> ()) {
        ...
        guard let detected = recognizer.dominantLanguage else {
            ...
            DispatchQueue.main.async {
                completion(.failure(TranslationError
                        .appleTranslation("Could not detect source language. Try selecting more text.")))
            }
            return
        }
...
final class TranslationBridge: ObservableObject {
    ...
    func translate(
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 10) { [weak self] in
            guard let self = self, translationID == thisID, pendingCompletion != nil else { return }
    ...
    private var pendingCompletion: ((Result<[String], Error>) -> ())?
...
enum TranslationError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .appleTranslation(msg): return msg
        }
    }
}
