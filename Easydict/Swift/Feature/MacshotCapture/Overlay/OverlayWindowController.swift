...
// MARK: - ScreenshotOverlayRootView
...
protocol OverlayWindowControllerDelegate: AnyObject {
    ...
    /// Host-driven exit: the composited image is delivered without a clipboard write.
...
class OverlayWindowController {
    ...
    private(set) var screen: NSScreen = .main ?? NSScreen.screens.first ?? NSScreen()
    ...
    /// Set flag so the composited image is handed to the host right after
    /// selection (Easydict OCR / dock-translate flows; no clipboard write).
    func setAutoHostImageMode() {
        overlayView?.autoHostImageMode = true
    }
    ...
    func updateScrollCaptureProgress(
        stripCount: Int,
        pixelSize: CGSize,
    ...
    private func playCopySound() {
        ...
        CaptureSound.sound?.stop()
        CaptureSound.sound?.play()
    }
    ...
    private func compositeAnnotationsOnSnappedWindow(
...
extension OverlayWindowController: OverlayViewDelegate {
    ...
    /// Host-driven exit: deliver the composited image without touching the
    /// clipboard — the host (OCR/translate) owns the pasteboard afterwards.
    func overlayViewDidRequestHostImage() {
        ...
        playCopySound()
        dismiss()
        overlayDelegate?.overlayDidRequestHostImage(self, image: image)
    }
    ...
    func overlayViewDidRequestOCR() {
        // Route the toolbar OCR exit to the host flow: Easydict owns OCR and
        // handles the recognized text itself, so just deliver the image.
        overlayViewDidRequestHostImage()
    }
    ...
    func overlayViewDidRequestShare(anchorView: NSView?) {
        ...
        let delegate = SharePickerDelegate(
            onPick: { [weak self] in
                ...
                overlayWindow?.level = savedLevel
                shareDelegate = nil
                playCopySound()
                let img = image
                dismiss()
                overlayDelegate?.overlayDidConfirm(self, capturedImage: img, annotationData: annotationData)
            },
    ...
    func overlayViewDidRequestDetach() {
        // The detached editor window is not part of this port; the in-overlay
        // editor covers annotation editing, so this exit stays a no-op.
    }
...
private class SharePickerDelegate: NSObject, NSSharingServicePickerDelegate {
    ...
    init(onPick: @escaping () -> (), onDismiss: @escaping () -> ()) {
        self.onPick = onPick
