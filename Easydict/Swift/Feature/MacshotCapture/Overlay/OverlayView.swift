...
protocol OverlayViewDelegate: AnyObject {
    ...
    /// Host-driven exit (Easydict OCR/translate): hand the composited image to
    /// the host right after selection without writing the clipboard.
    func overlayViewDidRequestHostImage()
...
enum UndoEntry {
    ...
    var annotation: Annotation {
        switch self {
        case let .added(a), let .deleted(a, _): return a
        case let .propertyChange(a, _): return a
        case .imageTransform:
            return Annotation(
                tool: .measure, startPoint: .zero, endPoint: .zero, color: .clear, strokeWidth: 0
            ) // dummy
        }
    }
}

// MARK: - OverlayEditorState
...
class OverlayView: NSView {
    ...
    var externalScreenshotPreviewUpdater: ((CGImage?) -> ())?
    ...
    var beautifyMode: BeautifyMode =
        .init(rawValue: UserDefaults.standard.integer(forKey: "beautifyMode")) ?? .window
    ...
    // Image effects
    var effectsPreset: ImageEffectPreset =
        .init(rawValue: UserDefaults.standard.integer(forKey: "effectsPreset")) ?? .none
    var effectsBrightness: Float = {
    ...
    var currentMeasureClampToSelection: Bool = UserDefaults.standard
        .object(forKey: "measureClampToSelection") as? Bool ?? true
    var currentLineStyle: LineStyle =
        .init(rawValue: UserDefaults.standard.integer(forKey: "currentLineStyle")) ?? .solid
    var currentArrowStyle: ArrowStyle =
        .init(rawValue: UserDefaults.standard.integer(forKey: "currentArrowStyle")) ?? .single
    var arrowReversed: Bool =
        UserDefaults.standard.bool(forKey: "arrowReversed")
    var currentRectFillStyle: RectFillStyle =
        .init(rawValue: UserDefaults.standard.integer(forKey: "currentRectFillStyle"))
            ?? .stroke
    ...
    var autoTranslateOverlayMode: Bool =
        false // set by macshot://ocr-translate — OCR + translate + overlay after selection
    ...
    var autoScrollCaptureMode: Bool =
        false // set by "Scroll Capture" menu — triggers scroll capture immediately after selection
    var autoConfirmMode: Bool = false // set by "Add Capture" — auto-confirms selection (no toolbars, no save)
    var autoHostImageMode: Bool =
        false // set by host apps — hands the image to the host after selection, no clipboard write
    ...
    var selectionPixelSize: (w: Int, h: Int) {
        ...
        return (
            Int((selectionRect.width * scale).rounded()),
    ...
    var toolHasOptionsRow: Bool {
        ...
        switch currentTool {
        case .arrow, .ellipse, .highlight, .line, .loupe, .marker, .measure, .number, .pencil,
             .pixelate, .rectangle, .stamp:
            return true
        case .text:
            return true
        default:
    ...
    override func mouseMoved(with event: NSEvent) {
           ...
           !showBeautifyInOptionsRow {
            ...
            if let previewPoint, let stampPreviewPoint {
                shouldMovePreview = hypot(
                    ...
                    previewPoint.y - stampPreviewPoint.y
                ) > 0.5
            } else {
        ...
        if state == .idle, windowSnapEnabled, !windowSnapQueryInFlight,
           !(remoteSelectionRect.width >= 1 && remoteSelectionRect.height >= 1) {
            guard let screenPoint = window.map({
    ...
    override func draw(_ dirtyRect: NSRect) {
        timingMark?(
            "OverlayView.draw begin state=\(state) screenshot=\(screenshotImage != nil) dirty=\(Int(dirtyRect.width))x\(Int(dirtyRect.height)) opaque=\(isOpaque)"
        )
        defer {
        ...
        if state != .idle, selectionRect.width >= 1, selectionRect.height >= 1 {
            ...
            // The EditorView subclass (detached editor) is not part of this port.
            let editorDrawnFromCache = false
            ...
            if !editorDrawnFromCache {
                ...
                if !annotations.isEmpty, !isEditorMode {
                    ...
                    for annotation in annotations
                        where annotation.tool != .translateOverlay && annotation.tool == .pixelate {
                        annotation.draw(in: context)
                    }
            ...
            if (currentTool == .pencil || currentTool == .marker) && drawingCursorPoint != .zero && currentAnnotation ==
                nil && !isDraggingAnnotation && !isResizingAnnotation && !isRotatingAnnotation {
            ...
            let showEffectsPreview = effectsActive && state == .selected && !isScrollCapturing && !isRecording &&
                !beautifyEnabled
            ...
            if showBeautifyPreview {
                ...
                if currentTool == .pencil || currentTool == .marker, drawingCursorPoint != .zero,
                   currentAnnotation == nil, !isDraggingAnnotation, !isResizingAnnotation, !isRotatingAnnotation {
                    context.saveGraphicsState()
                    applyCanvasTransform(to: context)
                    drawDrawingCursorPreview(at: drawingCursorPoint)
                    context.restoreGraphicsState()
            ...
            if showEffectsPreview, let screenshot = screenshotImage {
                ...
                if (currentTool == .pencil || currentTool == .marker) && drawingCursorPoint != .zero &&
                    currentAnnotation == nil && !isDraggingAnnotation && !isResizingAnnotation &&
            ...
            if let sv = textEditor.scrollView, textEditView != nil {
                ...
                let handleRects = [
                    NSRect(
                        x: sv.frame.minX - hs / 2, y: sv.frame.minY - hs / 2, width: hs, height: hs
                    ), // bottom-left
                    NSRect(
                        x: sv.frame.maxX - hs / 2, y: sv.frame.minY - hs / 2, width: hs, height: hs
                    ), // bottom-right
                    NSRect(
                        x: sv.frame.minX - hs / 2, y: sv.frame.maxY - hs / 2, width: hs, height: hs
                    ), // top-left
                    NSRect(
                        x: sv.frame.maxX - hs / 2, y: sv.frame.maxY - hs / 2, width: hs, height: hs
                    ), // top-right
                    NSRect(
                        x: sv.frame.midX - hs / 2, y: sv.frame.minY - hs / 2, width: hs, height: hs
                    ), // bottom
                    NSRect(
                        x: sv.frame.midX - hs / 2, y: sv.frame.maxY - hs / 2, width: hs, height: hs
                    ), // top
                    NSRect(
                        x: sv.frame.minX - hs / 2, y: sv.frame.midY - hs / 2, width: hs, height: hs
                    ), // left
                    NSRect(
                        x: sv.frame.maxX - hs / 2, y: sv.frame.midY - hs / 2, width: hs, height: hs
                    ), // right
                ]
    ...
    func startScrollCaptureMode() {
        ...
        let handleScrollKey: (NSEvent) -> () = { [weak self] event in
    ...
    func updateCursorForChrome(at point: NSPoint) -> Bool { false }
    ...
    func updateResolutionBox() {
        ...
        if let existing = resolutionBox {
            ...
            box.onCommit = { [weak self] w, h, edited in
                ...
                if applyDisplaySize(w: w, h: h, edited: edited) {
                    clearStaleExactPreSelectionPresetIfNeeded()
                }
            }
    ...
    func editorZoom(by factor: CGFloat, cursorInWindow: NSPoint, animated: Bool = false) {
        ...
        if animated {
            ...
            if editorZoomAnimTimer == nil {
                editorZoomAnimTimer = Timer
                    .scheduledTimer(withTimeInterval: 1.0 / 60.0, repeats: true) { [weak self] timer in
                        guard let self = self, let sv = enclosingScrollView else {
                            timer.invalidate()
                            return
                        }
    ...
    func boundarySnappedMovedRect(_ rect: NSRect, modifiers: NSEvent.ModifierFlags) -> NSRect {
        ...
        if let hit = index.nearestVertical(
            toViewX: rect.minX,
            yMinView: rect.minY,
            yMaxView: rect.maxY,
            radiusPoints: radius
        ...
        if let hit = index.nearestHorizontal(
            toViewY: rect.minY,
            xMinView: rect.minX,
            xMaxView: rect.maxX,
            radiusPoints: radius
    ...
    func handleToolbarAction(_ action: ToolbarButtonAction, mousePoint: NSPoint = .zero) {
        ...
        case .color:
            ...
            let colorBtn = bottomStripView?.buttonViews
                .first { if case .color = $0.action { return true }; return false }
        ...
        case .moveSelection:
            ...
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                clearToolbarHoverState(suppressUntilMouseMoved: true)
                setToolbarHoverSuppressed(false)
        ...
        case .share:
            ...
            let shareBtn = rightStripView?.buttonViews
                .first { if case .share = $0.action { return true }; return false }
        ...
        case .recordSettings:
            let gearBtn = rightStripView?.buttonViews
                .first { if case .recordSettings = $0.action { return true }; return false }
    ...
    func applyGlyphStrokeToLiveTextView() {
        ...
        if range.length > 0 {
            OutlineTextRenderer.applyOutline(
                enabled ? textEditor.glyphStrokeColor : nil,
                to: storage,
                range: range
            )
        }
    ...
    func opacityAppliedColor(for tool: AnnotationTool) -> NSColor {
        switch tool {
        case .blur, .loupe, .marker, .measure, .pixelate, .translateOverlay:
            return currentColor
        default:
    ...
    func undo() {
        ...
        case let .propertyChange(ann, snapshot):
            ...
            cachedCompositedImage = nil
        case let .imageTransform(previousImage, _):
    ...
    func redo() {
        ...
        case let .propertyChange(ann, snapshot):
            ...
            cachedCompositedImage = nil
        case let .imageTransform(redoImage, _):
    ...
    private var cachedAnnotationLayer: NSImage?
    /// During drag/resize, this holds a cache of all annotations EXCEPT the ones being manipulated.
    private var cachedAnnotationLayerExcludingSelected: NSImage?
    ...
    private var activeRatioLabel: String? {
        ...
        return ResolutionPresetCatalog.ratios.first {
            if case let .ratio(_, v) = $0 { return abs(v - a) < 0.001 }
            return false
        }?.label
    }
    ...
    private var activePreSelectionRatio: CGFloat? {
        if case let .ratio(aspect) = activePreSelectionPreset, aspect > 0 { return aspect }
        return nil
    }
    ...
    private var preSelectionPresetDisplayLabel: String? {
        ...
        case .freeform:
            ...
            return nil
        case let .ratio(aspect):
            return ResolutionPresetCatalog.ratios.first {
                if case let .ratio(_, value) = $0 { return abs(value - aspect) < 0.001 }
                return false
            }?.label ?? ratioLabel(for: aspect)
        case let .resolution(w, h):
            return ResolutionPresetCatalog.resolutions.first {
                if case let .resolution(_, presetW, presetH) = $0 {
    ...
    @objc
    private func handleToolbarColorsChanged() {
        ...
        if state == .selected, !isDraggingAnnotation, !isResizingAnnotation, !isRotatingAnnotation {
            ...
            if selectedAnnotation != nil {
                ...
                let isShapeTool = [
                    ...
                    .loupe,
                    .highlight,
                ].contains(selectedAnnotation?.tool)
                for (_, handleEntry) in annotationResizeHandleRects.enumerated() {
                    ...
                    if rect.insetBy(dx: -4, dy: -4).contains(handlePoint) {
                        if isShapeTool {
                            switch handle {
                            case .bottomRight, .topLeft: Self.nwseCursor.set()
                            case .bottomLeft, .topRight: Self.neswCursor.set()
    ...
    private func cursorForHandle(_ handle: ResizeHandle) -> NSCursor {
        switch handle {
        case .bottomRight, .topLeft: return Self.nwseCursor
        case .bottomLeft, .topRight: return Self.neswCursor
        case .bottom, .top: return .resizeUpDown
        case .left, .right: return .resizeLeftRight
        case .move, .none: return .arrow
        }
    }
    ...
    private func showResolutionPresets(from anchor: NSView) {
            ...
            return ResolutionPresetsView.Row(title: preset.label, isSelected: selected) { [weak self] in
                ...
                if keepRatioForNextCaptures {
                    if let aspect = preset.aspectValue {
                        setPreSelectionPreset(.ratio(aspect))
                    } else {
                ...
                applyLockedAspect(preset.aspectValue)
                persistRatioIfNeeded()
            }
        }
        ...
        if let curAspect = currentSelectionAspect {
            ...
            let customRow = ResolutionPresetsView.Row(title: customTitle, isSelected: customSelected) { [weak self] in
                PopoverHelper.dismiss()
                guard let self else { return }
                let aspect = currentSelectionAspect ?? curAspect
        ...
        view.resolutionRows = ResolutionPresetCatalog.resolutions.map { preset in
            guard case let .resolution(_, w, h) = preset else {
                return ResolutionPresetsView.Row(title: preset.label, isSelected: false, action: {})
            }
            return ResolutionPresetsView.Row(
                title: preset.label,
                isSelected: preSelectionPreset(activePreset, selects: preset)
            ) { [weak self] in
                PopoverHelper.dismiss()
                guard let self else { return }
                ...
                if keepRatioForNextCaptures {
                    setPreSelectionPreset(.resolution(w: w, h: h))
                } else {
                    setPreSelectionPreset(.freeform)
                }
                applyLockedAspect(nil)
                applyPixelSize(w: w, h: h)
                persistRatioIfNeeded()
            }
        }
        ...
        view.onToggleKeepRatio = { [weak self] on in
            guard let self else { return }
            keepRatioForNextCaptures = on
            persistRatioIfNeeded()
            refreshResolutionAndToolbarLayout() // refresh the enforced-icon tint
        }
    ...
    private func preSelectionPreset(_ activePreset: PreSelectionPreset, selects preset: ResolutionPreset) -> Bool {
        ...
        case (.freeform, .freeform):
            return true
        case let (.ratio(active), .ratio(_, value)):
            return abs(active - value) < 0.001
        case let (.resolution(activeW, activeH), .resolution(_, w, h)):
    ...
    private func clearStaleExactPreSelectionPresetIfNeeded() {
        guard case let .resolution(presetW, presetH) = activePreSelectionPreset else { return }
        let px = selectionPixelSize
    ...
    @objc
    private func preSelectionPresetButtonClicked(_ sender: NSButton) {
    ...
    private func showPreSelectionResolutionPresets(from anchor: NSView) {
        ...
        view.ratioRows = ResolutionPresetCatalog.ratios.map { preset in
            ...
            return ResolutionPresetsView.Row(title: preset.label, isSelected: selected) { [weak self] in
                ...
                case let .ratio(_, value):
                    setPreSelectionPreset(.ratio(value))
                default:
                    break
                }
            }
        ...
        view.onToggleKeepRatio = { [weak self] on in
            guard let self else { return }
            keepRatioForNextCaptures = on
            if on, case let .ratio(aspect) = activePreSelectionPreset {
                persistedAspect = aspect
            } else if !on {
                persistedAspect = 0
            }
            refreshResolutionAndToolbarLayout()
    ...
    private func drawBeautifyPreview(context: NSGraphicsContext) {
        ...
        let cornerRadius = config.isWindowSnap ? 10 : config
            .cornerRadius // native macOS corner radius for snapped windows
            ...
            if shadowRadius > 0 {
                ...
                context.cgContext.setShadow(
                    ...
                    color: NSColor.black.withAlphaComponent(
                        BeautifyRenderer.contactShadowAlpha(for: shadowRadius)
                    ).cgColor
                )
                ...
                context.cgContext.setShadow(
                    ...
                    blur: shadowRadius,
                    color: NSColor.black.withAlphaComponent(
                        BeautifyRenderer.shadowAlpha(for: shadowRadius)
                    ).cgColor
                )
                if let windowImg = snappedWindowImage {
                    windowImg.draw(in: imageRect, from: .zero, operation: .sourceOver, fraction: 1.0)
    ...
    private func startBeautifyToolbarAnimation() {
        ...
        beautifyToolbarAnimTimer = Timer.scheduledTimer(withTimeInterval: 1.0 / 60.0, repeats: true)
            { [weak self] timer in
                ...
                beautifyToolbarAnimProgress += 0.08 // ~12 frames = 0.2s
                if beautifyToolbarAnimProgress >= 1.0 {
                    beautifyToolbarAnimProgress = 1.0
                    timer.invalidate()
    ...
    private func opaqueContentRect(of cgImage: CGImage, scale: CGFloat) -> NSRect {
        ...
        let alphaOffset: Int
        switch alphaInfo {
        case .first, .noneSkipFirst, .premultipliedFirst: alphaOffset = 0
        case .last, .noneSkipLast, .premultipliedLast: alphaOffset = 3
    ...
    private func drawAnnotationControls(for annotation: Annotation, fullControls: Bool = true) {
        ...
        if annotation.tool == .loupe, let src = annotation.loupeSourceRect, src.width > 4 {
            ...
            let lensCenter = NSPoint(
                x: annotation.loupeLensSquareRect.midX,
    ...
    private func drawAnnotationOutlineGlow(_ annotation: Annotation) {
        ...
        offscreen.lockFocus()
        guard let offNSCtx = NSGraphicsContext.current
        else { annotation.rotation = savedRotation; offscreen.unlockFocus(); return }
    ...
    private func drawDeleteCircle(in rect: NSRect) {
        ...
        if let xImg = NSImage(systemSymbolName: "xmark", accessibilityDescription: nil)?
            ...
            tinted.draw(
                in: NSRect(
                    x: rect.midX - xImg.size.width / 2,
                    y: rect.midY - xImg.size.height / 2,
                    width: xImg.size.width,
                    height: xImg.size.height
                ),
                from: .zero,
        ...
        if let trashImg = NSImage(systemSymbolName: "trash", accessibilityDescription: nil)?
            ...
            tinted.draw(
                in: NSRect(
                    x: pillX + hPad,
                    y: iconY,
                    width: trashImg.size.width,
                    height: trashImg.size.height
                ),
                from: .zero,
                operation: .sourceOver,
                fraction: 1.0
    ...
    private func finishSelection() {
        if selectionRect.width > 5 || selectionRect.height > 5 {
            ...
            if !autoOCRMode, !autoQuickSaveMode, !autoScrollCaptureMode, !autoConfirmMode,
               !autoHostImageMode { showToolbars = true }
            overlayDelegate?.overlayViewDidFinishSelection(selectionRect)
                    ...
                    if let cgImage = await ScreenCaptureManager.captureWindow(windowID: wid, screen: screen) {
                        self.snappedWindowImage = NSImage(
                            ...
                            size: NSSize(
                                ...
                                height: CGFloat(cgImage.height) / screen
                                    .backingScaleFactor
                            )
                        )
                        self.needsDisplay = true
        ...
        // Host-driven exit — composited image goes to the host app directly.
        if autoHostImageMode {
            autoHostImageMode = false
            overlayDelegate?.overlayViewDidRequestHostImage()
        }
        needsDisplay = true
    }
    ...
    private func updateSelectionRect(
        ...
        if case let .resolution(pxW, pxH) = activePreSelectionPreset {
    ...
    private func applyBoundarySnap(
        ...
        if movesLeft, let hit = index.nearestVertical(
            toViewX: minX,
            yMinView: minY,
            yMaxView: maxY,
            radiusPoints: radius
        ...
        if movesRight, let hit = index.nearestVertical(
            toViewX: maxX,
            yMinView: minY,
            yMaxView: maxY,
            radiusPoints: radius
        ...
        if movesBottom, let hit = index.nearestHorizontal(
            toViewY: minY,
            xMinView: minX,
            xMaxView: maxX,
            radiusPoints: radius
        ...
        if movesTop, let hit = index.nearestHorizontal(
            toViewY: maxY,
            xMinView: minX,
            xMaxView: maxX,
            radiusPoints: radius
    ...
    private func constrainToAspect(_ rect: NSRect, aspect: CGFloat, handle: ResizeHandle, minSize: CGFloat) -> NSRect {
        ...
        switch handle {
        case .bottom, .top: w = h * aspect // height driven
    ...
    private func handleToolbarButtonHover(_ action: ToolbarButtonAction, hovered: Bool, strip: ToolbarStripView?) {
        ...
        if hovered {
            let btn = strip?.buttonViews.first { bv in
                ...
                if case let .tool(t1) = bv.action, case let .tool(t2) = action { return t1 == t2 }
    ...
    private func showKeystrokeModeMenu(anchorView: NSView) {
        ...
        let shortcutsItem = NSMenuItem(
            title: L("Shortcuts Only"),
            action: #selector(keystrokeModeShortcuts),
            keyEquivalent: ""
        )
    ...
    @objc
    private func keystrokeModeShortcuts() {
        UserDefaults.standard.set(false, forKey: "keystrokeShowAll")
    }

    @objc
    private func keystrokeModeAll() {
    ...
    private func showMicDeviceMenu(anchorView: NSView) {
        ...
        for device in devices {
            let item = NSMenuItem(
                ...
                action: #selector(micMenuSelectDevice(_:)),
                keyEquivalent: ""
            )
            item.target = self
            item.representedObject = device.uniqueID
            if micOn,
               savedUID == device.uniqueID || (savedUID == nil && device == AVCaptureDevice.default(for: .audio)) {
                item.state = .on
            }
            menu.addItem(item)
        }
    ...
    @objc
    private func micMenuSelectDevice(_ sender: NSMenuItem) {
    ...
    private func checkMicPermission(then next: @escaping () -> ()) {
    ...
    private func showWebcamSetupPreview() {
        ...
        let position = WebcamPosition(rawValue: UserDefaults.standard
            .string(forKey: "webcamPosition") ?? "bottomRight") ?? .bottomRight
    ...
    private func showCameraPermissionAlert() {
        ...
        alert
            .informativeText =
            L("macshot needs camera permission for the webcam overlay. Open System Settings to grant access.")
    ...
    private func showWebcamDeviceMenu(anchorView: NSView) {
        ...
        for device in devices {
            let item = NSMenuItem(
                title: device.localizedName,
                action: #selector(webcamMenuSelectDevice(_:)),
                keyEquivalent: ""
            )
            item.target = self
            item.representedObject = device.uniqueID
            if webcamOn,
               savedUID == device.uniqueID || (savedUID == nil && device == AVCaptureDevice.default(for: .video)) {
                item.state = .on
            }
            menu.addItem(item)
        }
    ...
    @objc
    private func webcamMenuSelectDevice(_ sender: NSMenuItem) {
        guard let uid = sender.representedObject as? String else { return }
    ...
    private func updateToolbarColorSwatch() {
        if let idx = bottomButtons
            .firstIndex(where: { if case .color = $0.action { return true } else { return false } }) {
    ...
    private func startAnnotation(at point: NSPoint) {
        ...
        if isPencilOrMarker, !ctrlHeld, !drawThrough {
            let hasAnnotationUnder = annotations.reversed()
                .contains(where: { $0.isMovable && $0.hitTest(point: point) })
    ...
    @objc
    private func saveAsMenuAction() {
        overlayDelegate?.overlayViewDidRequestSaveAs()
    }

    @objc
    private func saveToFolderMenuAction() {
...
// MARK: TextEditingCanvas

extension OverlayView: TextEditingCanvas {}

// MARK: - TooltipBackgroundView
