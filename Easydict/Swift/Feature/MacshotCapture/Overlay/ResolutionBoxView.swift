...
final class ResolutionBoxView: NSView, NSTextFieldDelegate {
    ...
    init() {
        ...
        presetsButton.image = NSImage(
            systemSymbolName: "aspectratio",
            accessibilityDescription: L("Aspect ratio & resolution presets")
        )
    ...
    var onCommit: ((_ w: Int, _ h: Int, _ edited: EditedDimension) -> ())?
    ...
    var onPresets: ((_ anchor: NSView) -> ())?

    /// Natural size of the control.
    var preferredSize: NSSize {
        NSSize(
            width: pad + fieldW + gap + timesWidth + gap + fieldW + gap + btnW + pad,
            height: fieldH + pad * 2
        )
    }
